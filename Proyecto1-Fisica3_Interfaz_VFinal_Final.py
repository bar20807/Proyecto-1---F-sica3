...
Al momento de querer trabajar con las figuras Lissajous, se determinó utilizar e implementar diferentes valores que representan 
el número de desfase original que viene en la guía del proyecto. Siendo estos los siguientes:
    - 0 es representado como 0
    - pi/4 es representado como 0.25
    - pi/2 es representado como 0.5
    - 3pi/4 es representado como 0.75
    - pi es representado como 1
A pesar de no ser el valor original, nosotros mediante la implementación de librerías como math y numpy pudimos establecer que cuando
el usuairo ingrese ese tipo de valores automáticamente el programa reconozca su equivalente en pi, y por lo tanto, mostrar la figura 
Lissajous correcta.
    ...
    if(I == 1):
        ...
        def update(i):
            if(i < len(x)+1):
                ...
                if (i >= 20):
                    ax.scatter(x[i-20],y[i-20],color='w',edgecolors=None)
                plt.title("Pantalla")
                plt.xlim(x[0],x[-1])
                if(y[-1] > 0):
                    plt.ylim(y[0],y[-1])
...
Delta_Titulo=Label(Ventana,text=("Desfase"))
Delta_Titulo.place(x=490,y=150)

#Creando los Scale
sclBarra=Scale(Ventana, label="Voltaje horizontal",orient=HORIZONTAL,length=140,from_=-9,to=9)
sclBarra.place(x=80,y=100)
sclBarra_1=Scale(Ventana,label="Voltaje vertical",orient=HORIZONTAL,length=140,from_=-9,to=9)
sclBarra_1.place(x=80,y=30)
sclBarra_2=Scale(Ventana, label="Voltaje de aceleración",orient=HORIZONTAL,length=140,from_=30,to=100)
...
#Creando los botones
btn_Estatico=Button(Ventana,text=("Generar gráfica estática"),command=static_click) 
btn_Estatico.place(x=90,y=250)
btn_Dinámico=Button(Ventana,text=("Genera gráfica dinámica"),command=dinamic_click) 
